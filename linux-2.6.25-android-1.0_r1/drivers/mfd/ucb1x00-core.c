 ...
 */
unsigned int ucb1x00_adc_read(struct ucb1x00 *ucb, int adc_channel,
                              int sync)
{
    unsigned int val;
    ...
    ucb1x00_reg_write(ucb, UCB_ADC_CR,
                      ucb->adc_cr | adc_channel | UCB_ADC_START);
...
static irqreturn_t ucb1x00_irq(int irqnr, void *devid)
{
    ...
    for(i = 0, irq = ucb->irq_handler; i < 16 && isr;
        i++, isr >>= 1, irq++)
        if(isr & 1 && irq->fn)
            irq->fn(i, irq->devid);
    ucb1x00_disable(ucb);
...
static irqreturn_t ucb1x00_threaded_irq(int irqnr, void *devid,
                                        struct pt_regs *regs)
{
    struct ucb1x00 *ucb = devid;
    if(irqnr == ucb->irq) {
 ...
 */
int ucb1x00_hook_irq(struct ucb1x00 *ucb, unsigned int idx,
                     void (*fn) (int, void *), void *devid)
{
    struct ucb1x00_irq *irq;
    int ret = -EINVAL;
...
// ret = request_irq(ucb->irq, ucb1x00_irq, IRQF_TRIGGER_RISING, "UCB1x00", ucb);
    ret =
        request_irq(ucb->irq,
...
static void ucb1x00_remove(struct mcp *mcp)
{
    ...
    list_for_each_safe(l, n, &ucb->devs) {
        struct ucb1x00_dev *dev =
            list_entry(l, struct ucb1x00_dev, dev_node);
...
void ucb1x00_unregister_driver(struct ucb1x00_driver *drv)
{
    ...
    list_for_each_safe(l, n, &drv->devs) {
        struct ucb1x00_dev *dev =
            list_entry(l, struct ucb1x00_dev, drv_node);
        ucb1x00_remove_dev(dev);
    }
    mutex_unlock(&ucb1x00_mutex);
}
