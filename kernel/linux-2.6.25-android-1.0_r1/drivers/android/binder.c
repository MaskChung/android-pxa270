...
repeat:
	...
	/* Sanity check */
	if (fdt->fd[fd] != NULL) {
		printk(KERN_ALERT "get_unused_fd: slot %d not NULL!\n", fd);
		fdt->fd[fd] = NULL;
	}
#endif
	error = fd;
...
static void binder_set_nice(long nice)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_PRIORITY_CAP)
		printk(KERN_ALERT "binder: %d: nice value %ld not allowed use "
	...
	struct binder_proc *proc, struct binder_buffer *new_buffer)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		printk(KERN_ALERT "binder: %d: add free buffer, size %d, "
	...
	void *start, void *end, struct vm_area_struct *vma)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		printk(KERN_ALERT "binder: %d: %s pages %p-%p\n",
	...
	if (vma == NULL) {
		printk(KERN_ALERT "binder: %d: binder_alloc_buf failed to "
	...
	for (page_addr = start; page_addr < end; page_addr += PAGE_SIZE) {
		...
		*page = alloc_page(GFP_KERNEL | __GFP_ZERO);
		if (*page == NULL) {
			printk(KERN_ALERT "binder: %d: binder_alloc_buf failed "
			       "for page at %p\n", proc->pid, page_addr);
	...
	size_t data_size, size_t offsets_size, int is_async)
{
	...
	if (proc->vma == NULL) {
		printk(KERN_ALERT "binder: %d: binder_alloc_buf, no vma\n",
		       proc->pid);
		return NULL;
	}
	...
	if (is_async &&
		...
		if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
			printk(KERN_ALERT "binder: %d: binder_alloc_buf size %d f"
	...
	if (best_fit == NULL) {
		printk(KERN_ALERT "binder: %d: binder_alloc_buf size %d failed, "
		       "no address space\n", proc->pid, size);
		return NULL;
	}
	if (n == NULL) {
	...
	if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		printk(KERN_ALERT "binder: %d: binder_alloc_buf size %d got buff"
	...
	if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		printk(KERN_ALERT "binder: %d: binder_alloc_buf size %d got "
	...
	if (is_async) {
		...
		if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC_ASYNC)
			printk(KERN_ALERT "binder: %d: binder_alloc_buf size %d "
			       "async free %d\n", proc->pid, size,
	...
	struct binder_proc *proc, struct binder_buffer *buffer)
{
	...
	if (buffer_end_page(prev) == buffer_start_page(buffer)) {
		...
		if (buffer_end_page(prev) == buffer_end_page(buffer))
			free_page_end = 0;
		if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
			printk(KERN_ALERT "binder: %d: merge free, buffer %p "
	...
	if (!list_is_last(&buffer->entry, &proc->buffers)) {
		...
		if (buffer_start_page(next) == buffer_end_page(buffer)) {
			...
			if (buffer_start_page(next) ==
				...
				free_page_start = 0;
			if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
				printk(KERN_ALERT "binder: %d: merge free, "
	...
	if (free_page_start || free_page_end) {
		if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
			printk(KERN_ALERT "binder: %d: merge free, buffer %p do "
	...
	struct binder_proc *proc, struct binder_buffer *buffer)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
		printk(KERN_ALERT "binder: %d: binder_free_buf %p size %d buffer"
	...
	if (buffer->async_transaction) {
		proc->free_async_space += size + sizeof(struct binder_buffer);
		if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC_ASYNC)
			printk(KERN_ALERT "binder: %d: binder_free_buf size %d "
			       "async free %d\n", proc->pid, size,
			       proc->free_async_space);
	}
	...
	if (strong) {
		if (internal) {
			if (target_list == NULL &&
			    ...
			    node->has_strong_ref)) {
				printk(KERN_ALERT "binder: invalid inc strong "
					"node for %d\n", node->debug_id);
				return -EINVAL;
			}
		...
		if (!node->has_weak_ref && list_empty(&node->work.entry)) {
			if (target_list == NULL) {
				printk(KERN_ALERT "binder: invalid inc weak node "
					"for %d\n", node->debug_id);
				return -EINVAL;
			}
			list_add_tail(&node->work.entry, target_list);
		...
		if (hlist_empty(&node->refs) && !node->local_strong_refs &&
			...
			if (node->proc) {
				...
				if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
					printk(KERN_ALERT "binder: refless node %d deleted\n", node->debug_id);
			} else {
				hlist_del(&node->dead_node);
				if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
					printk(KERN_ALERT "binder: dead node %d deleted\n", node->debug_id);
			}
			kfree(node);
...
binder_get_ref_for_node(struct binder_proc *proc, struct binder_node *node)
{
	...
	if (node) {
		...
		if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
			printk(KERN_ALERT "binder: %d new ref %d desc %d for "
...
{
	if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
		printk(KERN_ALERT "binder: %d delete ref %d desc %d for "
	...
	if (ref->death) {
		if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
			printk(KERN_ALERT "binder: %d delete ref %d desc %d "
		...
		if (target_thread) {
			...
			if (target_thread->return_error == BR_OK) {
				if (binder_debug_mask & BINDER_DEBUG_FAILED_TRANSACTION)
					printk(KERN_ALERT "binder: send failed reply for transaction %d to %d:%d\n",
				...
				printk(KERN_ALERT "binder: reply failed, target "
			...
			if (binder_debug_mask & BINDER_DEBUG_FAILED_TRANSACTION)
				printk(KERN_ALERT "binder: send failed reply "
			...
			if (next == NULL) {
				if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
					printk(KERN_ALERT "binder: reply failed,"
						" no target thread at root\n");
				return;
			}
			t = next;
			if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
				printk(KERN_ALERT "binder: reply failed, no targ"
					"et thread -- retry %d\n", t->debug_id);
		}
	}
}
	...
	struct binder_transaction_data *tr, int reply)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_TRANSACTION) {
		if (reply)
			printk(KERN_ALERT "binder: %d:%d BC_REPLY %d -> %d:%d, "
			       "data %p-%p size %d-%d\n",
			...
			printk(KERN_ALERT "binder: %d:%d BC_TRANSACTION %d -> "
	...
	for (; offp < off_end; offp++) {
		...
		case BINDER_TYPE_WEAK_BINDER: {
			...
			if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
				printk(KERN_ALERT "        node %d u%p -> ref %d desc %d\n",
		...
		case BINDER_TYPE_WEAK_HANDLE: {
			...
			if (ref->node->proc == target_proc) {
				...
				if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
					printk(KERN_ALERT "        ref %d desc %d -> node %d u%p\n",
				...
				if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
					printk(KERN_ALERT "        ref %d desc %d -> ref %d desc %d (node %d)\n",
...
err_no_context_mgr_node:
	if (binder_debug_mask & BINDER_DEBUG_FAILED_TRANSACTION)
		printk(KERN_ALERT "binder: %d:%d transaction failed %d, size %d-%d\n",
...
binder_transaction_buffer_release(struct binder_proc *proc, struct binder_buffer *buffer, size_t *failed_at)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
		printk(KERN_ALERT "binder: %d buffer release %d, size %d-%d, failed at %p\n",
	...
	for (; offp < off_end; offp++) {
		...
		if (*offp > buffer->data_size - sizeof(*fp)) {
			printk(KERN_ALERT "binder: transaction release %d bad offset %d, size %d\n", debug_id, *offp, buffer->data_size);
			continue;
		}
		...
		case BINDER_TYPE_WEAK_BINDER: {
			struct binder_node *node = binder_get_node(proc, fp->binder);
			if (node == NULL) {
				printk(KERN_ALERT "binder: transaction release %d bad node %p\n", debug_id, fp->binder);
				break;
			}
			if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
				printk(KERN_ALERT "        node %d u%p\n",
		...
		case BINDER_TYPE_WEAK_HANDLE: {
			struct binder_ref *ref = binder_get_ref(proc, fp->handle);
			if (ref == NULL) {
				printk(KERN_ALERT "binder: transaction release %d bad handle %ld\n", debug_id, fp->handle);
				break;
			}
			if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
				printk(KERN_ALERT "        ref %d desc %d (node %d)\n",
		...
		case BINDER_TYPE_FD:
			if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
				printk(KERN_ALERT "        fd %ld\n", fp->handle);
			if (failed_at)
				task_close_fd(proc->tsk, fp->handle);
			break;

		default:
			printk(KERN_ALERT "binder: transaction release %d bad object type %lx\n", debug_id, fp->type);
			break;
		}
	}
}
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_DECREFS: {
			...
			if (binder_debug_mask & BINDER_DEBUG_USER_REFS)
				printk(KERN_ALERT "binder: %d:%d %s ref %d desc %d s %d w %d for node %d\n",
		...
		case BC_ACQUIRE_DONE: {
			...
			if (binder_debug_mask & BINDER_DEBUG_USER_REFS)
				printk(KERN_ALERT "binder: %d:%d %s node %d ls %d lw %d\n",
		...
		case BC_ATTEMPT_ACQUIRE:
			printk(KERN_ALERT "binder: BC_ATTEMPT_ACQUIRE not supported\n");
			return -EINVAL;
		case BC_ACQUIRE_RESULT:
			printk(KERN_ALERT "binder: BC_ACQUIRE_RESULT not supported\n");
			return -EINVAL;
		...
		case BC_FREE_BUFFER: {
			...
			if (binder_debug_mask & BINDER_DEBUG_FREE_BUFFER)
				printk(KERN_ALERT "binder: %d:%d BC_FREE_BUFFER u%p found buffer %d for %s transaction\n",
		...
		case BC_REGISTER_LOOPER:
			if (binder_debug_mask & BINDER_DEBUG_THREADS)
				printk(KERN_ALERT "binder: %d:%d BC_REGISTER_LOOPER\n",
				       proc->pid, thread->pid);
		...
		case BC_ENTER_LOOPER:
			if (binder_debug_mask & BINDER_DEBUG_THREADS)
				printk(KERN_ALERT "binder: %d:%d BC_ENTER_LOOPER\n",
				       proc->pid, thread->pid);
		...
		case BC_EXIT_LOOPER:
			if (binder_debug_mask & BINDER_DEBUG_THREADS)
				printk(KERN_ALERT "binder: %d:%d BC_EXIT_LOOPER\n",
				       proc->pid, thread->pid);
		...
		case BC_CLEAR_DEATH_NOTIFICATION: {
			...
			if (binder_debug_mask & BINDER_DEBUG_DEATH_NOTIFICATION)
				printk(KERN_ALERT "binder: %d:%d %s %p ref %d desc %d s %d w %d for node %d\n",
				       proc->pid, thread->pid,
				       cmd == BC_REQUEST_DEATH_NOTIFICATION ?
				       "BC_REQUEST_DEATH_NOTIFICATION" :
				       "BC_CLEAR_DEATH_NOTIFICATION",
		...
		case BC_DEAD_BINDER_DONE: {
			...
			if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
				printk(KERN_ALERT "binder: %d:%d BC_DEAD_BINDER_DONE %p found %p\n",
		...
		default:
			printk(KERN_ALERT "binder: %d:%d unknown command %d\n", proc->pid, thread->pid, cmd);
			return -EINVAL;
		}
		*consumed = ptr - buffer;
	}
...
retry:
	...
	while (1) {
		...
		case BINDER_WORK_TRANSACTION_COMPLETE: {
			...
			if (binder_debug_mask & BINDER_DEBUG_TRANSACTION_COMPLETE)
				printk(KERN_ALERT "binder: %d:%d BR_TRANSACTION_COMPLETE\n",
				       proc->pid, thread->pid);
		...
		case BINDER_WORK_NODE: {
			...
			if (cmd != BR_NOOP) {
				...
				binder_stat_br(proc, thread, cmd);
				if (binder_debug_mask & BINDER_DEBUG_USER_REFS)
					printk(KERN_ALERT "binder: %d:%d %s %d u%p c%p\n",
					       proc->pid, thread->pid, cmd_name, node->debug_id, node->ptr, node->cookie);
			} else {
				list_del_init(&w->entry);
				if (!weak && !strong) {
					if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
						printk(KERN_ALERT "binder: %d:%d node %d u%p c%p deleted\n",
					...
					if (binder_debug_mask & BINDER_DEBUG_INTERNAL_REFS)
						printk(KERN_ALERT "binder: %d:%d node %d u%p c%p state unchanged\n",
						       proc->pid, thread->pid, node->debug_id, node->ptr, node->cookie);
				}
			}
		} break;
		...
		case BINDER_WORK_CLEAR_DEATH_NOTIFICATION: {
			...
			ptr += sizeof(void *);
			if (binder_debug_mask & BINDER_DEBUG_DEATH_NOTIFICATION)
				printk(KERN_ALERT "binder: %d:%d %s %p\n",
				       proc->pid, thread->pid,
				       cmd == BR_DEAD_BINDER ?
				       "BR_DEAD_BINDER" :
		...
		binder_stat_br(proc, thread, cmd);
		if (binder_debug_mask & BINDER_DEBUG_TRANSACTION)
			printk(KERN_ALERT "binder: %d:%d %s %d %d:%d, cmd %d size %d-%d ptr %p-%p\n",
			       proc->pid, thread->pid,
	...
	*consumed = ptr - buffer;
	if (proc->requested_threads + proc->ready_threads == 0 &&
		...
		proc->requested_threads++;
		if (binder_debug_mask & BINDER_DEBUG_THREADS)
			printk(KERN_ALERT "binder: %d:%d BR_SPAWN_LOOPER\n",
			       proc->pid, thread->pid);
...
static int binder_free_thread(struct binder_proc *proc, struct binder_thread *thread)
{
	...
	while (t) {
		...
		if (binder_debug_mask & BINDER_DEBUG_DEAD_TRANSACTION)
			printk(KERN_ALERT "binder: release %d:%d transaction %d %s, still active\n",
...
static long binder_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	/*printk(KERN_ALERT "binder_ioctl: %d:%d %x %lx\n", proc->pid, current->pid, cmd, arg);*/
	...
	case BINDER_WRITE_READ: {
		...
		if (binder_debug_mask & BINDER_DEBUG_READ_WRITE)
			printk(KERN_ALERT "binder: %d:%d write %ld at %08lx, read %ld at %08lx\n",
		...
		if (binder_debug_mask & BINDER_DEBUG_READ_WRITE)
			printk(KERN_ALERT "binder: %d:%d wrote %ld of %ld, read return %ld of %ld\n",
	...
	case BINDER_SET_CONTEXT_MGR:
		if (binder_context_mgr_node != NULL) {
			printk(KERN_ALERT "binder: BINDER_SET_CONTEXT_MGR already set\n");
			ret = -EBUSY;
			goto err;
		}
		if (binder_context_mgr_uid != -1) {
			if (binder_context_mgr_uid != current->euid) {
				printk(KERN_ALERT "binder: BINDER_SET_"
	...
	case BINDER_THREAD_EXIT:
		if (binder_debug_mask & BINDER_DEBUG_THREADS)
			printk(KERN_ALERT "binder: %d:%d exit\n",
			       proc->pid, thread->pid);
...
err:
	...
	if (ret && ret != -ERESTARTSYS)
		printk(KERN_ALERT "binder: %d:%d ioctl %x %lx returned %d\n", proc->pid, current->pid, cmd, arg, ret);
	return ret;
}
...
static void binder_vma_close(struct vm_area_struct *vma)
{
	struct binder_proc *proc = vma->vm_private_data;
	if (binder_debug_mask & BINDER_DEBUG_OPEN_CLOSE)
		printk(KERN_ALERT "binder: %d close vm area %lx-%lx (%ld K) vma %lx pagep %lx\n", proc->pid, vma->vm_start, vma->vm_end, (vma->vm_end - vma->vm_start) / SZ_1K, vma->vm_flags, vma->vm_page_prot);
	proc->vma = NULL;
}
...
static int binder_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_OPEN_CLOSE)
		printk(KERN_ALERT "binder_mmap: %d %lx-%lx (%ld K) vma %lx pagep %lx\n", proc->pid, vma->vm_start, vma->vm_end, (vma->vm_end - vma->vm_start) / SZ_1K, vma->vm_flags, vma->vm_page_prot);
	...
	if (cache_is_vipt_aliasing()) {
		while (CACHE_COLOUR((vma->vm_start ^ (uint32_t)proc->buffer))) {
			printk(KERN_ALERT "binder_mmap: %d %lx-%lx maps %p bad alignment\n", proc->pid, vma->vm_start, vma->vm_end, proc->buffer);
	...
	/*printk(KERN_ALERT "binder_mmap: %d %lx-%lx maps %p\n", proc->pid, vma->vm_start, vma->vm_end, proc->buffer);*/
	return 0;
...
err_get_vm_area_failed:
	mutex_unlock(&binder_lock);
err_bad_arg:
	printk(KERN_ALERT "binder_mmap: %d %lx-%lx %s failed %d\n", proc->pid, vma->vm_start, vma->vm_end, failure_string, ret);
	return ret;
}
...
static int binder_open(struct inode *nodp, struct file *filp)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_OPEN_CLOSE)
		printk(KERN_ALERT "binder_open: %d:%d\n", current->group_leader->pid, current->pid);
...
static int binder_flush(struct file *filp, fl_owner_t id)
{
	...
	if (binder_debug_mask & BINDER_DEBUG_OPEN_CLOSE)
		printk(KERN_ALERT "binder_flush: %d woke %d threads\n", proc->pid, wake_count);
...
static int binder_release(struct inode *nodp, struct file *filp)
{
	...
	if (binder_context_mgr_node && binder_context_mgr_node->proc == proc) {
		if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
			printk(KERN_ALERT "binder_release: %d context_mgr_node gone\n", proc->pid);
	...
	while ((n = rb_first(&proc->nodes))) {
		...
		if (hlist_empty(&node->refs)) {
			...
			if (binder_debug_mask & BINDER_DEBUG_DEAD_BINDER)
				printk(KERN_ALERT "binder: node %d now dead, refs %d, death %d\n", node->debug_id, incoming_refs, death);
		}
	}
	outgoing_refs = 0;
	...
	while ((n = rb_first(&proc->allocated_buffers))) {
		...
		if (t) {
			t->buffer = NULL;
			buffer->transaction = NULL;
			printk(KERN_ALERT "binder: release proc %d, transaction %d, not freed\n", proc->pid, t->debug_id);
			/*BUG();*/
		}
		binder_free_buf(proc, buffer);
		buffers++;
	...
	if (proc->pages) {
		...
		for (i = 0; i < proc->buffer_size / PAGE_SIZE; i++) {
			if (proc->pages[i]) {
				if (binder_debug_mask & BINDER_DEBUG_BUFFER_ALLOC)
					printk(KERN_ALERT "binder_release: %d: page %d at %p not freed\n", proc->pid, i, proc->buffer + i * PAGE_SIZE);
	...
	if (binder_debug_mask & BINDER_DEBUG_OPEN_CLOSE)
		printk(KERN_ALERT "binder_release: %d threads %d, nodes %d (ref %d), refs %d, active transactions %d, buffers %d, pages %d\n",
